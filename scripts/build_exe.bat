...
::   4. Build dist/data_pipeline/data_pipeline.exe
::
:: Output: dist\data_pipeline\data_pipeline.exe (+ _internal\ alongside)
:: Hand the whole dist\data_pipeline\ folder to the operator.
...
%PY% -m PyInstaller data_pipeline.spec --noconfirm --clean
if errorlevel 1 (
...
:: --- 5. Report ---
if exist "dist\data_pipeline\data_pipeline.exe" (
  echo.
  echo ====================================
  echo   Build succeeded
  echo ====================================
  for %%S in ("dist\data_pipeline\data_pipeline.exe") do echo   exe: %%~zS bytes
  echo   path: %CD%\dist\data_pipeline\data_pipeline.exe
  echo.
  echo Hand the whole dist\data_pipeline\ folder to the operator.
  echo The exe will NOT work on its own - _internal\ must stay next to it.
) else (
  echo ERROR: exe not found at dist\data_pipeline\data_pipeline.exe
  pause
  exit /b 1
)
echo.

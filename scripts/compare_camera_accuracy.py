...
Alternatively ``--metashape`` accepts an Agisoft camera-model
estimated-coordinates CSV export in WGS84 (already georeferenced, so no
similarity fit is needed); exactly one of the two must be given.
...
``extracted_frame_times.csv``), by projecting each frame's GPS position
onto the ``--pos`` track, or from times embedded in the reconstruction image
names themselves: a camera-model->COLMAP bundle (``sparse/0``) whose frames are
named ``frame_<t>_<idx>`` is self-timing (``--name-time-base`` says whether
``<t>`` is video-relative, boottime or UTC seconds; ``--use-name-times``
promotes those over the anchor/projection fallbacks).
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        ...
        epilog="Metashape: Reference pane -> Export Reference -> "
               "estimated coordinates -> CSV (WGS84). A Metashape->COLMAP "
               "bundle (sparse/0) whose frames are named frame_<t>_<idx> is "
               "self-timing: the embedded <t> is used when no other "
               "frame-time source resolves (see --name-time-base / "
               "--use-name-times).",
    )
    cam = p.add_mutually_exclusive_group(required=True)
    cam.add_argument(
        "--colmap", type=Path,
    ...
    cam.add_argument(
        "--metashape", type=Path,
        help="Agisoft Metashape estimated-coordinates CSV export in WGS84 "
             "(Reference pane -> Export Reference -> estimated coordinates "
             "-> CSV); used instead of --colmap",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--name-time-base", choices=("video", "boot", "utc"), default="video",
        help="What the time embedded in frame_<t>_<idx> image names is: "
             "video-relative seconds (needs --session for the boot->UTC "
             "anchor), CLOCK_BOOTTIME seconds, or absolute UTC seconds "
             "(default: video)",
    )
    p.add_argument(
        "--use-name-times", action="store_true",
        help="Prefer the times embedded in frame_<t>_<idx> image names over "
             "the session-anchor / track-projection fallbacks (they are "
             "used automatically when nothing else resolves)",
    )
    p.add_argument(
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    camera_path = args.metashape if args.metashape is not None else args.colmap
    result = build_report(
        camera_path,
        args.gt_pos,
        args.out,
        camera_source="metashape" if args.metashape is not None else "colmap",
        georef_csv=args.georef_csv,
        pos=args.pos,
        frame_times=args.frame_times,
        session_dir=args.session,
        name_time_base=args.name_time_base,
        use_name_times=args.use_name_times,

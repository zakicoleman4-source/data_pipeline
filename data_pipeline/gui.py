...
class App:
    ...
    _TAB_SPEC = [
        ...
        ("medium",  "_build_accuracy_tab"),
        ("medium",  "_build_frame_export_tab"),
    ...
    def _build_ppk_tab(self, nb: ttk.Notebook) -> None:
        ...
        # Bundled presets (data_pipeline/configs/) are always shown; merge in
        # any extra .conf from an external solver dir if one was configured.
        presets = list(ppk_stage.list_packaged_configs())
        _seen = {p.name for p in presets}
        for extra in ppk_stage.list_config_files():
            if extra.name not in _seen:
                presets.append(extra)
                _seen.add(extra.name)
        ...
        _Tooltip(self._ppk_preset_cb,
                 "Presets bundled with the app (plus any from a configured "
    ...
    def _build_analysis_tab(self, nb: ttk.Notebook) -> None:
        ...
        ttk.Label(
            g_cam,
            text=(
                "Compare a COLMAP reconstruction or a Metashape estimated-"
                "coords CSV against the "
        ...
        self.var_cam_source = tk.StringVar(value="colmap")
        _cam_src = ttk.Frame(g_cam)
        _cam_src.pack(anchor="w", padx=8, pady=(0, 2))
        ttk.Label(_cam_src, text="Camera source:").pack(side="left")
        ttk.Radiobutton(_cam_src, text="COLMAP reconstruction",
                        value="colmap", variable=self.var_cam_source).pack(
                            side="left", padx=(6, 0))
        ttk.Radiobutton(_cam_src, text="Metashape CSV (WGS84 estimated)",
                        value="metashape", variable=self.var_cam_source).pack(
                            side="left", padx=(6, 0))
        btn_cam = ttk.Button(
        ...
        _Tooltip(btn_cam,
                 "COLMAP: asks for images.txt/.bin (or the reconstruction\n"
                 "folder). Metashape: asks for the estimated-coords CSV\n"
                 "(Reference pane -> Export Reference -> estimated, WGS84).\n"
                 "Plus a ground-truth .pos; uses the pipeline's Georef.csv\n"
                 "when built this session (asks otherwise), then writes\n"
                 "camera_vs_gps_vs_gt.csv + an offline HTML report.")

        # ── Frame time table (all notations) ──────────────────────────────
        g_ft = ttk.LabelFrame(f, text="Frame time table (all notations)")
        g_ft.pack(fill="x", padx=10, pady=(4, 10))
        ttk.Label(
            g_ft,
            text=(
                "One row per extracted frame with every time notation side "
                "by side: video PTS, boot ns, UTC, ISO, GPST and seconds-"
                "from-audio-start. Writes frame_times_all.csv + a searchable "
                "HTML table."
            ),
            foreground="#888", wraplength=920, justify="left",
        ).pack(anchor="w", padx=8, pady=(6, 2))
        btn_ft = ttk.Button(
            g_ft, text="Frame time table (all notations)",
            command=self._run_frame_time_table,
        )
        btn_ft.pack(anchor="w", padx=8, pady=(2, 8))
        self._buttons.append(btn_ft)
        _Tooltip(btn_ft,
                 "Uses this session's extracted_frame_times.csv + RAW\n"
                 "session anchors to write frame_times_all.csv (Image,\n"
                 "video_pts_s, boot_ns, utc_s, utc_iso, gpst_s, t_audio_s)\n"
                 "and an offline searchable HTML table, then opens it.")
    ...
    def _build_accuracy_tab(self, nb: ttk.Notebook) -> None:
        """Compare one or more camera-model reconstructions against a
        ground-truth track, frame by frame, in one offline HTML report.
        Add a project per reconstruction (e.g. two exports from the same
        capture) and each becomes its own source in every table and chart."""
        f = self._make_scrollable_tab(nb, "Accuracy")

        if not hasattr(self, "var_ac_gt"):
            self.var_ac_gt = tk.StringVar()
            self.var_ac_track = tk.StringVar()
            self.var_ac_georef = tk.StringVar()
            self.var_ac_ftimes = tk.StringVar()
            self.var_ac_out = tk.StringVar()
            self.var_ac_epsg = tk.StringVar()
            self._ac_projects: list[tuple[str, str]] = []

        ttk.Label(
            f,
            text=(
                "Score camera-model reconstructions against a survey-grade "
                "ground-truth track. Pick the four shared inputs once, add one "
                "project per reconstruction (e.g. two exports from the same "
                "capture), then build a single offline HTML report: absolute "
                "and best-fit accuracy, robust and distribution statistics, "
                "confidence intervals, per-axis / along-cross / speed / heading "
                "error, a top-view map and error charts."
            ),
            foreground="#888", wraplength=920, justify="left",
        ).pack(anchor="w", padx=10, pady=(8, 4))

        def _pick(var, title, pattern):
            p = filedialog.askopenfilename(title=title, filetypes=pattern)
            if p:
                var.set(p)

        def _file_row(parent, label, var, title, tip, pattern, dirsel=False):
            row = ttk.Frame(parent)
            row.pack(fill="x", padx=8, pady=2)
            ttk.Label(row, text=label, width=30, anchor="w").pack(side="left")
            ent = ttk.Entry(row, textvariable=var)
            ...
            def _do():
                if dirsel:
                    ...
                    _pick(var, title, pattern)
            ttk.Button(row, text="Browse…", command=_do).pack(side="left", padx=(4, 0))
            _Tooltip(ent, tip)

        POS = [("Position .pos", "*.pos"), ("All", "*.*")]
        CSV = [("CSV", "*.csv"), ("All", "*.*")]
        box = ttk.LabelFrame(f, text="Shared inputs (one per capture)")
        box.pack(fill="x", padx=10, pady=(6, 4))
        _file_row(box, "Ground-truth track (.pos)", self.var_ac_gt,
                  "Pick the ground-truth .pos",
                  "Survey-grade reference trajectory (the truth).", POS)
        _file_row(box, "Device track (.pos)", self.var_ac_track,
                  "Pick the device .pos",
                  "Device track — used only to recover the frame-time offset.", POS)
        _file_row(box, "Per-frame coords (.csv)", self.var_ac_georef,
                  "Pick the per-frame coordinates CSV",
                  "Image,Latitude,Longitude,Altitude,... per frame.", CSV)
        _file_row(box, "Per-frame times (.csv)", self.var_ac_ftimes,
                  "Pick the per-frame times CSV",
                  "Image,t_video_s per frame.", CSV)

        pbox = ttk.LabelFrame(f, text="Camera-model projects (add one CSV per project)")
        pbox.pack(fill="both", expand=True, padx=10, pady=(6, 4))
        self._ac_tree = ttk.Treeview(pbox, columns=("label", "path"),
                                     show="headings", height=5)
        self._ac_tree.heading("label", text="Label")
        self._ac_tree.heading("path", text="cameras_est_computed.csv")
        self._ac_tree.column("label", width=140)
        self._ac_tree.column("path", width=560)
        self._ac_tree.pack(side="left", fill="both", expand=True, padx=(6, 0), pady=6)
        pbtn = ttk.Frame(pbox)
        pbtn.pack(side="left", fill="y", padx=6, pady=6)
        ttk.Button(pbtn, text="Add project…", command=self._ac_add_project).pack(fill="x", pady=2)
        ttk.Button(pbtn, text="Remove", command=self._ac_remove_project).pack(fill="x", pady=2)

        _file_row(f, "Output folder (optional)", self.var_ac_out,
                  "Pick the output folder",
                  "Where accuracy_report.html is written.\n"
                  "Defaults to the per-frame-coords folder.", CSV, dirsel=True)

        epsg_row = ttk.Frame(f)
        epsg_row.pack(fill="x", padx=8, pady=2)
        ttk.Label(epsg_row, text="Projected CRS EPSG (optional)", width=30,
                  anchor="w").pack(side="left")
        ent_epsg = ttk.Entry(epsg_row, textvariable=self.var_ac_epsg, width=12)
        ent_epsg.pack(side="left")
        _Tooltip(ent_epsg,
                 "Leave blank if the camera-model CSVs are WGS84 (lat/lon).\n"
                 "If they are in a projected/UTM CRS, enter its EPSG code\n"
                 "(e.g. 32636 for UTM 36N) to reproject to WGS84.")

        btn_row = ttk.Frame(f)
        btn_row.pack(fill="x", padx=10, pady=(6, 10))
        ttk.Button(btn_row, text="Build accuracy report",
                   command=self._run_accuracy_report).pack(side="left")

    def _ac_add_project(self) -> None:
        from tkinter import simpledialog
        fn = filedialog.askopenfilename(
            title="camera-model estimate CSV",
            filetypes=[("CSV", "*.csv"), ("All", "*.*")])
        if not fn:
            return
        default = Path(fn).parent.name or Path(fn).stem
        label = simpledialog.askstring("Project label",
                                       "Label for this project:", initialvalue=default)
        if not label:
            return
        self._ac_projects.append((label, fn))
        self._ac_tree.insert("", "end", values=(label, fn))

    def _ac_remove_project(self) -> None:
        for iid in self._ac_tree.selection():
            vals = self._ac_tree.item(iid, "values")
            self._ac_projects = [p for p in self._ac_projects
                                 if not (p[0] == vals[0] and p[1] == vals[1])]
            self._ac_tree.delete(iid)

    def _run_accuracy_report(self) -> None:
        req = [(self.var_ac_gt, "ground-truth .pos"),
               (self.var_ac_track, "device track .pos"),
               (self.var_ac_georef, "per-frame coords .csv"),
               (self.var_ac_ftimes, "per-frame times .csv")]
        for var, name in req:
            s = var.get().strip()
            if not s or not Path(s).is_file():
                messagebox.showerror("Missing input", f"Pick the {name} first.")
                return
        if not self._ac_projects:
            if not messagebox.askyesno(
                    "No projects",
                    "No camera-model project added. Compare device track vs "
                    "ground truth only?"):
                return
        out_s = self.var_ac_out.get().strip()
        out_dir = Path(out_s) if out_s else Path(self.var_ac_georef.get()).parent
        out_html = out_dir / "accuracy_report.html"
        gt, track = self.var_ac_gt.get(), self.var_ac_track.get()
        georef, ftimes = self.var_ac_georef.get(), self.var_ac_ftimes.get()
        epsg = self.var_ac_epsg.get().strip()
        if epsg and not epsg.isdigit():
            messagebox.showerror("Bad EPSG", f"EPSG must be a number, got {epsg!r}.")
            return
        projects = list(self._ac_projects)
        script = Path(__file__).resolve().parent.parent / "scripts" / "accuracy_report.py"

        def go() -> None:
            # Run the report IN-PROCESS (same interpreter as the app, which
            # already has numpy + the data_pipeline package loaded) rather than
            # shelling out — no dependence on sys.executable, a loose script
            # path, or a second environment. The script uses argparse + writes
            # the HTML, so we set argv, capture its stdout/stderr into the log,
            # and translate its sys.exit() into a clear error.
            import runpy
            import io as _io
            import contextlib as _cl
            if not script.is_file():
                raise RuntimeError(
                    f"accuracy_report.py not found at {script} — the app must "
                    "run from a full source clone (it needs the script).")
            argv = ["accuracy_report.py",
                    "--gt", gt, "--track", track, "--georef", georef,
                    "--ftimes", ftimes, "--out", str(out_html)]
            if epsg:
                argv += ["--epsg", epsg]
            if projects:
                argv.append("--meta")
                argv += [f"{lbl}={path}" for lbl, path in projects]
            else:
                argv.append("--no-meta")
            ...
            # Always leave a log file next to the report so a failing run can
            # be diagnosed / sent on, whether it succeeded or not.
            log_path = out_html.with_name("accuracy_report.log")
            try:
            ...
            for line in buf.getvalue().strip().splitlines():
                self._log(f"[accuracy] {line}")
            self._log(f"[accuracy] full log: {log_path}")

            if err is not None:
                raise RuntimeError(f"accuracy report error: {err}\n(log: {log_path})")
            if code:
                tail = "\n".join(buf.getvalue().strip().splitlines()[-4:]) or "no output"
                raise RuntimeError(f"accuracy report failed:\n{tail}\n(log: {log_path})")
            if not out_html.is_file():
                raise RuntimeError(
                    f"report finished but {out_html.name} was not written "
                    f"(log: {log_path}).")
            self.root.after(0, lambda: self._open_path_in_default(out_html))

        self._run_async(go, "Accuracy report")

    def _build_frame_export_tab(self, nb: ttk.Notebook) -> None:
        """Per-frame trajectory CSV: join extracted frame times + per-frame
        coordinates into one client CSV with audio-relative + UTC time, WGS84
        and UTM position, and geographic ENU velocity (+ speed & azimuth)."""
        f = self._make_scrollable_tab(nb, "Frame export")

        if not hasattr(self, "var_fx_ftimes"):
            self.var_fx_ftimes = tk.StringVar()
            self.var_fx_georef = tk.StringVar()
            self.var_fx_session = tk.StringVar()
            self.var_fx_pos = tk.StringVar()
            self.var_fx_out = tk.StringVar()

        ttk.Label(
            f,
            text=(
                "One CSV per frame: audio-relative time + UTC, WGS84 "
                "(lat/lon/alt) AND UTM (auto zone) position, geographic "
                "East/North/Up velocity (Doppler AND coordinate-delta) with "
                "speed and true-north azimuth, and — when a phone PPK .pos is "
                "given — the weighted_v2-smoothed position (WGS84 + UTM) and "
                "velocity. UTC / audio time (and smoothing) need the raw "
                "session folder for the boot->UTC + audio anchors; without it "
                "the video-relative time is written, UTC is blank, and the "
                "smoothed columns stay empty. For a chopped clip, point the "
                "session at the CHOP so t_audio_s is chop-relative."
            ),
            foreground="#888", wraplength=920, justify="left",
        ).pack(anchor="w", padx=10, pady=(8, 4))

        def _row(label, var, title, tip, *, dirsel=False, pattern=None):
            row = ttk.Frame(f)
            row.pack(fill="x", padx=8, pady=2)
            ttk.Label(row, text=label, width=30, anchor="w").pack(side="left")
            ent = ttk.Entry(row, textvariable=var)
            ...
            def _do():
                p = (filedialog.askdirectory(title=title) if dirsel
                     else filedialog.askopenfilename(title=title, filetypes=pattern))
                if p:
                    var.set(p)
            ttk.Button(row, text="Browse…", command=_do).pack(side="left", padx=(4, 0))
            _Tooltip(ent, tip)
        ...
        CSV = [("CSV", "*.csv"), ("All", "*.*")]
        _row("Per-frame times (.csv)", self.var_fx_ftimes,
             "Pick extracted_frame_times.csv",
             "Image,t_video_s per frame.", pattern=CSV)
        _row("Per-frame coords (.csv)", self.var_fx_georef,
             "Pick the per-frame coordinates CSV",
             "Image,Latitude,Longitude,Altitude,...(+ DopplerVe/Vn/Vu for velocity).",
             pattern=CSV)
        _row("Raw session folder (for UTC/audio)", self.var_fx_session,
             "Pick the raw capture session folder (the CHOP for a trimmed clip)",
             "Optional. Provides the boot->UTC + audio-origin anchors so\n"
             "utc_s / utc_iso / t_audio_s are absolute. For a chopped clip\n"
             "point it at the CHOP so t_audio_s is chop-relative. Leave blank\n"
             "for video-relative time only (and no smoothed columns).", dirsel=True)
        _row("Phone PPK .pos (for smoothing)", self.var_fx_pos,
             "Pick the phone PPK .pos to smooth (weighted_v2)",
             "Optional. Enables the sm_* smoothed position (WGS84 + UTM) and\n"
             "velocity columns. Needs the session folder above (UTC) to align\n"
             "the .pos epochs to the frames.",
             pattern=[("POS", "*.pos"), ("All", "*.*")])
        _row("Output CSV (optional)", self.var_fx_out,
             "Pick the output CSV path",
             "Defaults to frame_export.csv next to the coordinates CSV.",
             pattern=CSV)

        btn_row = ttk.Frame(f)
        btn_row.pack(fill="x", padx=10, pady=(6, 10))
        ttk.Button(btn_row, text="Build trajectory CSV",
                   command=self._run_frame_export).pack(side="left")

    def _run_frame_export(self) -> None:
        for var, name in ((self.var_fx_ftimes, "per-frame times .csv"),
                          (self.var_fx_georef, "per-frame coords .csv")):
            s = var.get().strip()
            if not s or not Path(s).is_file():
                messagebox.showerror("Missing input", f"Pick the {name} first.")
                return
        ftimes = Path(self.var_fx_ftimes.get())
        georef = Path(self.var_fx_georef.get())
        session_s = self.var_fx_session.get().strip()
        session = Path(session_s) if session_s else (
            self.paths.raw_folder if getattr(self.paths, "raw_folder", None) else None)
        if session is not None and not Path(session).is_dir():
            messagebox.showerror("Bad session", f"Session folder not found: {session}")
            return
        pos_s = self.var_fx_pos.get().strip()
        pos_csv = Path(pos_s) if pos_s else None
        if pos_csv is not None and not pos_csv.is_file():
            messagebox.showerror("Bad .pos", f"Phone PPK .pos not found: {pos_csv}")
            return
        if pos_csv is not None and session is None:
            messagebox.showwarning(
                "Smoothing needs a session",
                "A phone .pos was given but no session folder — the smoothed "
                "columns need UTC (from the session) to align the .pos to the "
                "frames, so they will be left blank. Add the session folder to "
                "fill them.")
        out_s = self.var_fx_out.get().strip()
        out_csv = Path(out_s) if out_s else georef.parent / "frame_export.csv"

        def go() -> None:
            from .frame_export import build_frame_export
            out_csv.parent.mkdir(parents=True, exist_ok=True)
            build_frame_export(ftimes, georef, out_csv,
                               session_dir=session, pos_csv=pos_csv, log=self._log)
            self.root.after(0, lambda: self._open_path_in_default(out_csv))

        self._run_async(go, "Trajectory export")
    ...
    def _run_camera_report(self) -> None:
        ...
        """
        # 1) Camera input — COLMAP reconstruction or camera-model estimated CSV.
        cam_source = self.var_cam_source.get()
        if cam_source == "metashape":
            meta = filedialog.askopenfilename(
                title=("Pick the Metashape estimated-coords CSV "
                       "(Reference pane -> Export Reference -> estimated, WGS84)"),
                filetypes=[("CSV files", "*.csv"), ("All files", "*.*")],
            )
            if not meta:
                self._log("[camrep] cancelled (no Metashape CSV).")
                return
            cam_input = Path(meta)
            cam_kwargs = {"camera_source": "metashape", "metashape": cam_input}
        else:
            ...
            if not colmap:
                ...
                return
            cam_input = Path(colmap)
            cam_kwargs = {"camera_source": "colmap"}
        ...
        # build_report also needs a TIME source: the pipeline's Georef.csv
        # carries per-frame positions but NO time column, so frame times must
        # still be recovered from the rover .pos (track projection), the
        # session anchor, or a frame-times CSV. Always pass whatever is loaded
        # — even when a Georef is present. (Previously these were nulled
        # whenever a Georef existed, which crashed build_report with "cannot
        # determine per-frame UTC times" on any Georef without a time column.)
        session_dir = self.paths.raw_folder
        pos_path = self.paths.pos_path
    ...
    def _run_frame_time_table(self) -> None:
        """'Frame time table' — ``frame_time_table.build_frame_time_table``.

        Resolves this session's extracted_frame_times.csv + RAW anchors,
        writes frame_times_all.csv + a searchable HTML table off-thread,
        then opens the HTML.
        """
        ft = self.paths.frame_times_csv
        ses = self.paths.raw_folder
        if ft is None and ses is not None:
            cand = Path(ses) / "extracted_frame_times.csv"
            if cand.is_file():
                ft = cand
        if ft is None:
            s = filedialog.askopenfilename(
                title="Pick extracted_frame_times.csv",
                filetypes=[("CSV files", "*.csv"), ("All files", "*.*")],
            )
            ft = Path(s) if s else None
        if ft is None:
            messagebox.showerror(
                "Frame time table",
                "Need extracted_frame_times.csv (extract frames first).")
            return
        ft = Path(ft)
        out_s = self.var_an_out.get().strip()
        out_base = (self.paths.out_dir
                    or (Path(out_s) if out_s else ft.parent))
        out_csv = Path(out_base) / "frame_times_all.csv"
        sess_dir = Path(ses) if ses else None

        def go() -> None:
            from .frame_time_table import build_frame_time_table
            out_csv.parent.mkdir(parents=True, exist_ok=True)
            p = build_frame_time_table(
                ft, session_dir=sess_dir, out_csv=out_csv,
                write_html=True, log=self._log,
            )
            self.root.after(0, lambda: self._open_path_in_default(p))

        self._run_async(go, "Frame time table")

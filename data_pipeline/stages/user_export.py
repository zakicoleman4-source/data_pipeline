...
def resolve_export_rows(
    ...
    *,
    ...
    ``source``:
      * ``"raw"``  -> the raw post-processed rows, returned unchanged (new list, same
        row objects);
...
def export_trajectory(
    ...
    *,
    ...
    columns. ``final_v*`` is the RAW post-processed DOPPLER velocity (r.vn/ve/vu);
    ...
    * iso   -> ``utc_iso``   = ISO-8601 UTC string YYYY-MM-DDThh:mm:ss.sssZ
    ...
    if final_vel_enabled:
        # Opt-in final velocity block (raw post-processed Doppler + disagreement gate);

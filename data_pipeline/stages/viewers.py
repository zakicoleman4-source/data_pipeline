    ...
    if wav is not None and Path(wav).is_file():
        ...
        try:
            ...
            if v_t0_boot is None and video_anchor is not None \
                ...
                # Use the module-level helper (defined above). A local
                # ``from .georef import _first_boottime_ns_from_video_anchor``
                # here would make the name function-local for the WHOLE
                # function -> UnboundLocalError at the earlier chop-anchor use
                # -> audio offset silently stays 0 -> trimmed-clip audio ~200s
                # off. (Regression: keep this a bare call to the module fn.)

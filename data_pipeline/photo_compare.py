...
from .parsers import PosRow, interp_pos, parse_rtkpos
...
def _motion_fields(
    ...
    *,
    ...
    """Fill speed / azimuth fields from consecutive frames (in place).

    ``recs`` must be sorted by ``utc_s``. Azimuth errors are only produced
    where the ground-truth horizontal speed clears ``speed_floor_mps`` (a
    near-stationary motion vector has no meaningful bearing).
    """
    for i in range(1, len(recs)):
        a, b = recs[i - 1], recs[i]
        dt_s = b.utc_s - a.utc_s
        if not (0.0 < dt_s <= max_step_s):
            continue
        moves = {}
        for src in ("cam", "gps", "gt"):
            ...
            if not math.isfinite(du):
                du = 0.0
            moves[src] = (de, dn, du)
            speed = math.sqrt(de * de + dn * dn + du * du) / dt_s
            setattr(b, f"{src}_speed_mps", speed)
            if math.hypot(de, dn) >= _MIN_BEARING_DISP_M:
                setattr(b, f"{src}_az_deg", heading_from_enu(de, dn))
        gt_hspeed = math.hypot(moves["gt"][0], moves["gt"][1]) / dt_s
        for src in ("cam", "gps"):
            setattr(b, f"{src}_speed_err_mps",
                    getattr(b, f"{src}_speed_mps") - b.gt_speed_mps)
            dv = [(moves[src][k] - moves["gt"][k]) / dt_s for k in range(3)]
...
def build_report(
    ...
    *,
    ...
    _motion_fields(recs, speed_floor_mps=speed_floor_mps, max_step_s=max_step_s)
    agg = _aggregate(recs)
    ...
    if write_html:
        inputs = [f"camera model: {Path(colmap_images).name}",

...
def convert2rnx(args):
    ...
    sys.stderr = open(errFile, "w", encoding='utf-8', errors='replace')
    ...
    # find first valid epoch.  ``merge`` returns ``None`` for any batch where
    # every processed measurement was skipped (all-NaN row, glonass slot with
    # no freq channel, etc.), so we have to walk past those entries instead
    # of indexing them directly.  Same scan applied in reverse for lastepoch
    # — the previous ``batches[-1]['epoch'][0]`` crashed with
    # ``'NoneType' object is not subscriptable`` whenever the recording
    # ended on a skipped batch.
    firstepoch = None
    ...
    for batch in reversed(batches):
        if batch is None:
            continue
        try:
            lastepoch = batch['epoch'][0]
            break
        except (KeyError, IndexError, TypeError):
            continue

    if firstepoch is None or lastepoch is None:
        raise RuntimeError(
            "No valid epochs in {0}; the GNSS log has no usable Raw "
            "measurement rows (check the phone's GNSS settings — Raw "
            "measurements must be enabled on the recording device).".format(
                args.input_log)
        )
    ...
    # Filter out skipped-batch sentinels before formatting so a single
    # all-skipped row in the middle of the recording can't crash the run.
    body = ''.join(arinex.write_obs(b, obslist) for b in batches if b is not None)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--keep-level',  metavar='<strict|relaxed|permissive>',
                        type=str, default='relaxed',
                        choices=list(alogger.KEEP_LEVEL_PRESETS.keys()),
                        help="Signal-quality strictness."
                             " relaxed (default, phone-agnostic) = CNo>=15, SVT-unc<=1us,"
                             " ignore Android multipath flag (it lies on Samsung-class"
                             " devices), still drop ADR-cycle-slip rows. Verified +14-18%"
                             " more usable sat-obs vs strict on every phone we've tested."
                             " strict = Google decimeter-challenge thresholds (CNo>=20,"
                             " SVT-unc<=500ns, also drop multipath flag)."
                             " permissive = CNo>=10, SVT-unc<=2us, keep slip rows."
                             " Hard filters (code lock, TOW/TOD, known constellation) are"
                             " never disabled.")

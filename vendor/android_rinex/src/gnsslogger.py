#!/usr/bin/env python3
"""
Module to process log files from Google's Android satellite positioning Logger app
"""
...
# Quality-filter presets. The hard structural filters (code lock, TOW,
# constellation) are not configurable — disabling them produces garbage.
# These knobs only relax the *signal-quality* checks so noisier recordings
# still emit measurements; the post-processed engine then makes its own call about
# which to use.
#
#   strict     : Google decimeter-challenge defaults. Tight.
#   relaxed    : Drop CNo floor to 15 dB-Hz, allow 1 us SVT-uncert,
#                ignore Android multipath flag entirely, keep cycle-slip
#                measurements as-is.
#   permissive : Drop CNo floor to 10 dB-Hz, allow 2 us SVT-uncert,
#                disable multipath / slip / ADR-invalid rejection. Use
#                when the recording is the only one you will get; the
#                post-processed solver will reject what it can't use.
KEEP_LEVEL_PRESETS = {
    'strict':     {'min_cn0': 20.0, 'max_svt_unc_ns': 500.0,
                   'apply_multipath': True,  'apply_slip': True},
    'relaxed':    {'min_cn0': 15.0, 'max_svt_unc_ns': 1000.0,
                   'apply_multipath': False, 'apply_slip': True},
    'permissive': {'min_cn0': 10.0, 'max_svt_unc_ns': 2000.0,
                   'apply_multipath': False, 'apply_slip': False},
}
...
def filter_obs(measurement, model, keep_level='relaxed'):
    """
    Quality-gate a single Raw measurement.

    The HARD filters (code lock, TOW / TOD decoded, known constellation)
    are non-negotiable — disabling them produces garbage RINEX. Only the
    SIGNAL-QUALITY filters (CNo floor, SVT-uncertainty, multipath flag,
    ADR cycle slip) are tunable via ``keep_level``.

    keep_level (str): one of ``strict`` (default), ``relaxed``, or
        ``permissive``. See ``KEEP_LEVEL_PRESETS`` for what each one
        relaxes. Unknown values fall back to ``strict``.
    """

    cfg = KEEP_LEVEL_PRESETS.get(keep_level, KEEP_LEVEL_PRESETS['strict'])
    ...
    # HARD: known constellation
    if constellation == CONSTELLATION_UNKNOWN:
        raise ValueError('ConstellationType Unknown')

    # QUALITY: signal strength floor
    if measurement['Cn0DbHz'] < cfg['min_cn0']:
        raise ValueError("CNo below threshold")

    # QUALITY: range uncertainty
    if measurement['ReceivedSvTimeUncertaintyNanos'] > cfg['max_svt_unc_ns']:
        raise ValueError('ReceivedSvTime uncertainty exceeds threshold')

    # QUALITY: Android-reported multipath. Codes:
    #   0 = UNKNOWN, 1 = DETECTED, 2 = NOT_DETECTED.
    # Only ``DETECTED`` is a true positive — UNKNOWN / NOT_DETECTED are
    # the cases we want to keep. ``apply_multipath=False`` (relaxed and
    # permissive) skips the check entirely, useful when the phone
    # reports DETECTED too aggressively.
    if cfg['apply_multipath'] and measurement['MultipathIndicator'] == 1:
        raise ValueError("Multipath indicated")

    # QUALITY: ADR (carrier-phase) state — RESET (2) and CYCLE_SLIP (4)
    # mean the carrier-phase has discontinuities. Pseudorange is still
    # usable, so ``apply_slip=False`` keeps the row with the carrier-
    # phase column zeroed by downstream code.
    if cfg['apply_slip'] and (measurement['AccumulatedDeltaRangeState'] == 2
                              or measurement['AccumulatedDeltaRangeState'] == 4):
        raise ValueError('ADR State indicates cycle slip or invalid')
...
def get_satname(measurement):
    """
    Obtain the satellite name from a satellite positioning Logger measurement
...
def process(measurement, model, fix_bias=True, timeadj=1e-7, pseudorange_bias=0.0,
    ...
    :param measurement: Satellite positioning Logger measurement line to process
    ...
    # GnssClock sanity check. Some budget Android devices (sm-a205u family
    # observed in the field) emit Raw rows where FullBiasNanos is missing
    # (empty string) or pinned to 0.0 — both make the GPS-week computation
    # below either crash with ``TypeError: bad operand type for unary -:
    # 'str'`` or produce a bogus week 0. Skip the row cleanly instead of
    # letting the math wreck the whole conversion run. The list-comp in
    # gnsslogger_to_rnx.py is *not* wrapped in try/except, so we must
    # return None here rather than raise ValueError.
    try:
        _fb_check = float(fullbiasnanos)
    except (TypeError, ValueError):
        sys.stderr.write(
            "-- WARNING: FullBiasNanos missing/non-numeric ({0!r}) for satellite [ {1} ]\n".format(
                fullbiasnanos, satname)
        )
        return None
    if _fb_check == 0.0:
        sys.stderr.write(
            "-- WARNING: FullBiasNanos == 0 (phone did not initialise GnssClock) "
            "for satellite [ {0} ]\n".format(satname)
        )
        return None
    fullbiasnanos = _fb_check  # ensure numeric for the math below
    ...
    try:
        filter_obs(measurement, model, keep_level=keep_level)
    except ValueError as e:
    ...
    # in post-processing engine (u-blox) units
    if range != 0.0:

...
* Coordinate-source chooser: ``resolve_export_rows`` picks the exported
  trajectory (raw post-processed or a specific smoother's output) independently of the
...
* Final-velocity export + coord/Doppler disagreement gate:
  ...
  drop_coords_on_vel_disagree=...)`` appends ``final_v*`` (raw post-processed Doppler)
...
def test_final_velocity_columns_are_raw_doppler_when_no_threshold(tmp_path):
    ...
    assert len(data) == 20
    for d in data:
        # final_v* = raw post-processed Doppler, always populated (threshold None).

...
# ---------------------------------------------------------------------------
# Audio a/v offset: the shadowed-import UnboundLocalError regression
# ---------------------------------------------------------------------------

def test_sync_player_no_shadowed_boottime_import() -> None:
    """Regression (real client bug): a function-local
    ``from .georef import _first_boottime_ns_from_video_anchor`` inside
    ``build_sync_player`` made that name function-local for the WHOLE
    function -> UnboundLocalError at the EARLIER chop-anchor use (a/v offset
    branch) -> the audio try/except swallowed it -> ``audio_video_offset_ms``
    silently stayed 0.0 -> the player referenced the full-session wav at
    offset 0 -> a trimmed clip's audio was ~200 s misaligned.

    The module already defines ``_first_boottime_ns_from_video_anchor`` at
    module scope; ``build_sync_player`` must use THAT (a bare call), never a
    function-local import that shadows it.
    """
    import ast
    import textwrap

    import data_pipeline.stages.viewers as V

    assert hasattr(V, "_first_boottime_ns_from_video_anchor")
    # AST (not substring) so the fix's own explanatory comment doesn't trip it.
    tree = ast.parse(textwrap.dedent(inspect.getsource(V.build_sync_player)))
    bad = [
        node.lineno
        for node in ast.walk(tree)
        if isinstance(node, ast.ImportFrom)
        for alias in node.names
        if alias.name == "_first_boottime_ns_from_video_anchor"
    ]
    assert not bad, (
        f"build_sync_player has a function-local import of "
        f"_first_boottime_ns_from_video_anchor (line offset {bad}) -- it "
        "shadows the module-level helper and makes it unbound at the earlier "
        "chop a/v-offset use (UnboundLocalError -> audio offset silently 0 "
        "-> trimmed clip audio ~200 s off). Use the module-level helper."
    )
